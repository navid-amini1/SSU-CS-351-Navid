...
int main(int argc, char* argv[]) {
    ...
    for (size_t id = 0; id < threads.size(); ++id) {
          threads[id] = std::jthread{ [&, id, chunkSize, numSamples, partitions]() {      
    ...
    for (size_t i = 0; i < numThreads; ++i) {
        totalInside += insidePoints[i];
    }
    
    double volume = static_cast<double>(totalInside) / numSamples;
    std::cout << volume << "\n";
}
